...
if args.d != None:
    ...
    if product.find("iPad") == 0:
        ...
        if args.u:
            isFRD = input("Update Parameter Set Not Performing FDR, Continue ? y/n: ").lower().strip()
            if isFRD == 'y':
                is_non_FDR = "-u"
        if proceed == 'y':
            cmd = "%s" %fr + " -t %s " %args.t + "-m %s " %buildManifest + "-p %s " %buildManifest + "-s %s " %sep[0] + "--no-baseband " + "%s " %ipsw + "%s" %is_non_FDR
            if args.l:
                print("[DEBUG] %s " %cmd)
            os.system(cmd)
        if proceed == 'n':
            ...
            if args.u:
                cmd = "%s" %fr + " -t %s " %args.t + "-m %s " %buildManifest + "-p %s " %buildManifest + "-s %s " %sep[0] + "-b %s " %baseband  + "%s " %ipsw + "%s" %is_non_FDR
            if args.l:
                print("[DEBUG] %s " %cmd)
                os.system(cmd) 
