...
def download(url, path, version, product, log):
    ...
    if not os.path.isfile(ipsw):
        ...
        if log:
            print("\n[*] IPSW already exists for " + product +
                  " on iOS " + version + " at path: %s" % ipsw)
    ...
    if signedReq.status_code == 200:
        ...
        for i in range(apiLength):
            if api['firmwares'][i]['signed'] == True or version == None and api['firmwares'][i]['signed'] == True:
                print("[V] iOS", api['firmwares'][i]['version'],
                      "is currently being signed for the:", api['identifier'])
...
def restoreFileFetch(product, signed, homePath):
    with RemoteZip(data['Devices'][product]['Firmwares'][0][signed][0]['IPSW']) as zip:
        ...
        zip.extract(data['Devices'][product]['Firmwares']
                    [0][signed][0]['SEP'], homePath)
        try:
            zip.extract(data['Devices'][product]['Firmwares']
                        [0][signed][0]['Baseband'], homePath)
        except:
            print(
...
if platform != "Windows":
    parser.add_argument('-p', help='Set Custom Save Path for Downloaded Files',
                        default=os.path.expanduser('~/Desktop/RestoreMe'))
if platform == "Windows":
    parser.add_argument('-p', help='Set Custom Save Path for Downloaded Files',
                        default=os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop/RestoreMe'))
parser.add_argument(
    '-d', help='Download Restore Files Only', action='store_true')
parser.add_argument('-e', help='Exit Recovery Mode', action='store_true')
parser.add_argument(
    '-u', help='Set Update paramater, to keep user data, do not perform FDR', action='store_true')
parser.add_argument('-t', help='Set SHSH ticket used for the restore')
parser.add_argument(
    '-l', help='Set program to print all info', action='store_true')
...
# if not args.d and not args.e and not args.t and not args.u and not args.l and not args.p:
#     parser.print_help()
#     sys.exit(-1)
...
if platform == "Windows":
    try:
        udid = deviceExtractionTool(
            'ideviceinfo.exe', 16, 'UniqueDeviceID: ', False)
        ecid = deviceExtractionTool(
            'ideviceinfo.exe', 13, 'UniqueChipID: ', True)
        platform = deviceExtractionTool(
            'ideviceinfo.exe', 18, 'HardwarePlatform: ', False)
        product = deviceExtractionTool(
            'ideviceinfo.exe', 13, 'ProductType: ', False)
        user = deviceExtractionTool(
            'ideviceinfo.exe', 12, 'DeviceName: ', False)
        boardid = deviceExtractionTool(
...
if platform != "Windows":
    try:
        rec = 'ideviceenterrecovery %s' % udid
        print(rec)
        os.system(rec)
    except:
        print("Unabled to place device in Recovery Mode")

if platform == "Windows":
    try:
        rec = 'ideviceenterrecovery.exe %s' % udid
        os.system(rec)
    except:
        print("Unabled to place device in Recovery Mode")
...
sep = glob.glob(os.path.join(
    args.p + "/Files/Firmware/all_flash", f'sep-firmware.*.RELEASE.im4p'))
baseband = glob.glob(os.path.join(
    args.p + "/Files/Firmware", f'*.Release.bbfw'))
...
if args.d != None:
    ...
    if args.t == None:
        args.t = input(
            "Enter path to SHSH Ticket for the device you wish to restore: ")
    if product.find("iPad") == 0:
        print("[*] No Baseband set, adding no-baseband flag to the restore")
        proceed = input(
            "No Baseband provided, Continue with the Restore (fine for iPod and iPad) ? y/n: ").lower().strip()
        if args.u:
            isFRD = input(
                "Update Parameter Set Not Performing FDR, Continue ? y/n: ").lower().strip()
            if isFRD == 'y':
                is_non_FDR = "-u"
        if proceed == 'y':
            cmd = "%s" % fr + " -t %s " % args.t + "-m %s " % buildManifest + "-p %s " % buildManifest + \
                "-s %s " % sep[0] + "--no-baseband " + \
                "%s " % ipsw + "%s" % is_non_FDR
            if args.l:
                print("[DEBUG] %s " % cmd)
            os.system(cmd)
